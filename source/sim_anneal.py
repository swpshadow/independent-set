...
for i in range(0,5):
    ...
    #beta = 1.001/1.01
    alpha = .985
    best = data_set.fix_up([1 if random.random() < 0.5 else 0 for _ in range(0,set_size)])
    s = [x for x in best]
    since_improvement = 0
    start_time = time.time()

    while time.time() - start_time < 60:# and data_set.fitness(best) < optimal:
    ...
    print("final temp: ", temp, "avg num iterations: ", total_perts,"best fit: ", data_set.fitness(best))
