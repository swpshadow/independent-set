...
class DataSet:
    ...
    def random_pool(self, pool_size = 10):
        ...
        for _ in range(0, pool_size):
            c = [x for x in range(0, self.size)]
            random.shuffle(c)
            pool.append(c)
        return pool
    ...
    def fitness(self, chromosome):
        idx = 1
        while self.is_feasable(chromosome[:idx]):
            idx += 1
        return idx - 1

    #checks to see if chromosome is feasible
    def is_feasable(self, chromosome):
        for curr_idx in range(0, len(chromosome)):
            for prev_vert in chromosome[curr_idx:]:
                if self.data[chromosome[curr_idx]][prev_vert] == 1:
                    return False
        return True
